...
const Projects_s_m_l = ({ dataObj }) => {
  const [textViewItem, setTextViewItem] = useState(false);
  const [selectedProject, setSelectedProject] = useState({});

  const handleClickMore = (project) => {
    setTextViewItem(!textViewItem);
          ...
          <div className="projectCard" key={index}>
            <div className="headCard">
              ...
              {textViewItem ? (
                <>
                  {item.id === selectedProject.id ? (
                    <div className="project_see_details">
                      <h1 className="project_title_m">
                        {selectedProject.title}
                      </h1>
                      {selectedProject.description.map((des, index) => {
                        return (
                          <p key={index} className="desContent">
                            <span>
                              <b>&minus;</b>
                            </span>
                            {des}
                      ...
                      <div id="link_code">
                        ...
                        {selectedProject.git !== null ? (
                          <a href={selectedProject.git} target="_blank">
                            <Button2
                              text={"View Code"}
                              path={
                      ...
                      <button
                        className="desCloseBtn"
                        onClick={() => setTextViewItem(false)}
                      >
                        Close
                      </button>
                    </div>
