...
const Carousel1 = ({ prop }) => {
  ...
  const handleClickSeemore = (itemData) => {
  ...
  return (
    <>
      {textView ? (
        <div id="des">
          <h1 id="project_des_title"> {selectedItem.title}</h1>
          ...
          <div id="link_code">
            {selectedItem.liveLink !== null ? (
              <a href={selectedItem.liveLink} target="_blank">
                <Button2
                  text={"Go Live"}
                  path={
            ...
            {selectedItem.git !== null ? (
              <a href={selectedItem.git} target="_blank">
                <Button2
                  text={"View Code"}
                  path={
          ...
          <button id="desCloseBtn" onClick={() => setTextView(false)}>
            Close
          </button>
        </div>
      ) : (
      ...
      <div id="Carousel_container">
        ...
        <div id="slide" ref={slideRef}>
          {prop.map((item, index) => (
            <div
              key={item.id}
              className={`item ${textView ? "blurBg" : ""}`}
